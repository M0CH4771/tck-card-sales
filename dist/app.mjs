...
function updateUrl(){const f=filters(),p=new URLSearchParams();if(f.query)p.set('q',f.query);for(const k of controls)if(f[k]!== ({grade:'8',rarity:'all',source:'all',sort:'recent',setCode:'all',availability:'all'}[k]))p.set(k,f[k]);history.replaceState(null,'',location.pathname+(p.size?'?'+p.toString():'')+location.hash);}
...
function render(){
  ...
  $('count').textContent=`${visible.length.toLocaleString()}件`;$('export').disabled=!visible.some(p=>p.sales.length);
  ...
  $('results').innerHTML=visible.slice(0,shownLimit).map(p=>`<button class="result-card ${p.id===selectedId?'selected':''}" data-id="${escapeHtml(p.id)}" aria-pressed="${p.id===selectedId}"><span class="card-top"><span class="tag grade">PSA ${escapeHtml(p.grade)}</span><span class="tag rarity">${escapeHtml(p.rarity)}</span></span><div class="card-name">${escapeHtml(p.name)}</div><p class="card-subtitle">${escapeHtml(p.number)}${p.number?' · ':''}${escapeHtml(p.set||p.nameEn)}</p><div class="card-bottom">${p.latest?`<div class="card-price">${money(p.latest.price)}<small>USD</small></div><div class="card-date">${date(p.latest.date)}<span>${escapeHtml(p.latest.source)} · 直近の取引</span></div>`:`<div class="price-pending">${escapeHtml(availabilityLabel(p))}</div>`}</div></button>`).join('')+(visible.length>shownLimit?`<button class="button show-more" id="show-more">さらに50件を表示（${shownLimit} / ${visible.length}件）</button>`:'')+`<p class="result-note">成約履歴あり ${visible.filter(p=>p.latest).length}件 / 未確認・公開履歴なし ${visible.filter(p=>!p.latest).length}件</p>`;
  ...
  renderDetail(visible.find(p=>p.id===selectedId));
...
function clearFilters(){ $('search').value='';$('grade').value='8';for(const id of ['rarity','source','setCode','availability'])$(id).value='all';shownLimit=50;render();}
